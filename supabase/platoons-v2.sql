-- Golf Recon Platoons v2
-- My Time / Platoons foundation with Chief + Co-Chief roles, motto, season,
-- Missions, mission-level results, feed/media, Club Eagle and future Cup scoring.
--
-- IMPORTANT INVARIANT:
...
alter table public.golfrecon_platoons add column if not exists motto text;
alter table public.golfrecon_platoons add column if not exists season_year integer;
update public.golfrecon_platoons set season_year=extract(year from now())::integer where season_year is null;
alter table public.golfrecon_platoons alter column season_year set default extract(year from now())::integer;
...
-- Migrate the v1 role names if that schema was already applied.
update public.golfrecon_platoon_members set role='chief' where role='owner';
update public.golfrecon_platoon_members set role='co_chief' where role='admin';

alter table public.golfrecon_platoon_members drop constraint if exists golfrecon_platoon_members_role_check;
alter table public.golfrecon_platoon_members
  add constraint golfrecon_platoon_members_role_check
  check (role in ('chief','co_chief','member','cadet'));
...
create unique index if not exists golfrecon_one_active_chief_uq
  on public.golfrecon_platoon_members(platoon_id)
  where role='chief' and status='active';
create unique index if not exists golfrecon_one_active_cochief_uq
  on public.golfrecon_platoon_members(platoon_id)
  where role='co_chief' and status='active';
...
-- Current Golf Recon security architecture uses authenticated Edge Functions with
-- a service-role client and explicit membership/role checks. Keep these tables
-- unavailable to direct browser API access until dedicated membership-aware RLS
-- policies are intentionally introduced.
